...
import { PortkeyNameVersion } from 'constants/portkey';
const INITIAL_STATE = {
  isActive: false,
  account: undefined,
...
export function PortkeyReactProvider({ children }: PortkeyReactProviderProps) {
  ...
  const activate = useCallback(async () => {
    ...
    dispatch({
      ...
      payload: {
        ...
        isActive: true,
        networkType,
        aelfNodeList: Object.values(chainInfo).map((item) => ({ ...item[0], rpcUrl: item[0].endPoint })),
      },
    });
    // return bridges;
  }, []);
  ...
  const networkChanged = useCallback(
    async (networkType: NetworkType) => {
      if (state.networkType && networkType !== state.networkType) {
        deactivate();
      }
    },
    [deactivate, state.networkType],
  );

...
export interface IAElfNode {
  chainId: string;
  exploreUrl?: string;
  rpcUrl: string;
  caContractAddress?: string;
}
export interface IAElfNodes {
  [key: string]: IAElfNode;
}
export function useAEflConnect() {
  ...
  return useCallback(
    async (isConnectEagerly?: boolean, nodes = AElfNodes) => {
      try {
        const bridges = await (isConnectEagerly ? connectEagerly : activate)(nodes);
        if (bridges) {
...
export function usePortkey(): Web3Type {
  ...
  const tmpContext = useMemo(() => {
    ...
    return {
      ...
      connector: 'PORTKEY',
      isPortkey: true,
      portkeyProvider: contextNetwork.provider,
    };
  }, [portkeyReact]);
  return tmpContext;
}
